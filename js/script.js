...
document.addEventListener('DOMContentLoaded', () => {
   ...
   function initReviewPage() {
    ...
    // Set max date for date-used to today
    ...
    // Configure UI based on whether reviewing an existing perfume
    ...
    const checkFormValidity = () => {
        ...
        // Set currentDate to end of day to allow today's date
        ...
        // Clear any previous error message when form becomes valid
        if (isFormValid) {
    ...
    dateInput.addEventListener('change', () => {
        ...
        const currentDate = new Date();
        currentDate.setHours(23, 59, 59, 999); // Allow entire day
    ...
    async function renderReviews() {
        ...
        try {
            ...
            // Fetch product links (with fallback if it fails)
            let productLinks = {};
            try {
            ...
            // Group reviews by perfume title
            ...
            perfumeArray.forEach(([normTitle, data]) => {
                ...
                // Add Get One link only if product URL exists, and Add Your Review link
    ...
    async function initAdminPage() {
        ...
        try {
            ...
            // Render Approved Reviews Section
            ...
            // Render Sponsored Perfumes Section
            const perfumeMap = new Map();
            allApprovedReviews.forEach(review => {
                const normTitle = review.title?.toLowerCase().trim();
                if (!normTitle) return;
        ...
        grid.addEventListener('click', async (e) => {
            ...
            if (target.matches('.btn-approve, .btn-reject, .btn-delete')) {
                ...
                if (!reviewId || !category) {
                    // Handle contact message status update or deletion
                ...
                try {
                    ...
                    target.checked = !isSponsored; // Revert checkbox on error
                }
            }
        });
    }
